...
const SecureSessionConfiguration = () => {
  ...
  const downloadSecureBot = () => {
    ...
    const script = BotSecurity.generateHardenedBotScript(currentSessionId, session.pseudonym, enableTor);
  ...
  const downloadDockerSetup = () => {
    ...
    // Download Dockerfile
    const dockerBlob = new Blob([dockerfile], {
      type: 'text/plain'
    });
    ...
    const composeBlob = new Blob([compose], {
      type: 'text/plain'
    });
  ...
  return <div className="space-y-6">
      ...
      {/* Aktuelle Session */}
      {currentSessionId && <Card className="bg-gray-800 border-gray-700">
          <CardHeader>
          ...
          <form onSubmit={handleSecureSessionUpdate} className="space-y-4">
            <div>
              ...
              <div className="flex gap-2">
                <Input id="sessionId" type="text" value={sessionId} onChange={e => setSessionId(e.target.value)} className="bg-gray-700 border-gray-600 text-gray-100" placeholder="Sichere Session-ID" required minLength={32} />
            ...
            <div>
              ...
              <Input id="sessionPassword" type="password" value={sessionPassword} onChange={e => setSessionPassword(e.target.value)} className="bg-gray-700 border-gray-600 text-gray-100" placeholder="Passwort zur Bestätigung" required />
            </div>
            ...
            {sessionError && <div className="flex items-center gap-2 text-red-400 text-sm">
