...
const VendorBanner = () => {
  ...
  return <div className="bg-gray-800 rounded-lg p-6 mb-8 border border-gray-700">
      ...
      <div className="space-y-6">
        <div>
          ...
          <ImageUpload currentImage={bannerData.bannerImage} onImageChange={handleBannerImageChange} placeholder="Upload your shop banner" allowDownload={true} className="w-full max-w-md" />
        </div>

        <div>
          <Label className="text-gray-300 mb-3 block">Banner Visibility</Label>
          <RadioGroup value={bannerData.isVisible ? 'visible' : 'hidden'} onValueChange={handleVisibilityChange} className="flex gap-6">
        ...
        <div>
          <Button onClick={toggleInvisibleMode} variant={bannerData.invisibleMode ? "destructive" : "outline"} className={bannerData.invisibleMode ? "bg-red-600 hover:bg-red-700" : "border-gray-600 text-gray-300 hover:bg-gray-700"}>
        ...
        {bannerData.bannerImage && <div className="bg-gray-700 rounded-lg p-4">
            <h4 className="text-sm font-medium text-gray-300 mb-2">Preview</h4>
            <div className="bg-gray-900 rounded p-2">
              <img src={bannerData.bannerImage} alt="Banner Preview" className="w-full max-w-sm h-32 object-cover rounded" />
            </div>
          </div>}
      </div>
    </div>;
